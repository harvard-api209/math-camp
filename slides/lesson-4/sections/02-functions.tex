...
\begin{frame}[fragile]{Focus on economies above \$10,000 per person}
\begin{lstlisting}[language=R]
health_focus <- health |>
...
\end{lstlisting}
\small \texttt{select()} retains the variables needed for the comparison. \texttt{filter()} defines the analytical sample: the tables and figures that follow describe economies with GDP per capita, PPP, above \$10,000 in 2022.
\end{frame}
...
\end{lstlisting}
The output has one row per income group. The count and range describe the sample; the mean and median describe its center.
\end{frame}
