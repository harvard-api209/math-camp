...
::: {.activity}
Write the first action that another person should take.
...
Ask whether another person can recover the question, inputs, decisions, and result from the project record.

## Someone else's kitchen

A recipe is tested when another person can use:
...
<div class="claim-line">A clean run removes the computational equivalent of remembered ingredients.</div>
...
This practical standard gives the reviewer observable evidence in the current environment.
...
## Handoff clinic: a project that lies

Open `examples/handoff-clinic/`. It contains four deliberate failures:

1. an absolute path that works only on one computer,
2. an object used before it is created,
3. an output older than the code,
4. a causal sentence unsupported by the analysis.

Run `Rscript examples/handoff-clinic/analysis.R` from a clean session. Stop at
the first failure, record the evidence, and repair one issue at a time. The
instructor key is separate from the student fixture.

::: {.notes}
0:17-0:28. Let the first run fail. Students predict and record the first blocker before anyone edits. Repair the absolute path and hidden object in public, then compare the stale output and causal claim. This is a worked example of the four audit categories, not a generic warning.
:::
...
6. Repair the source so the change can be rerun.
...
<div class="claim-line">A flight recorder makes a failure reconstructable.</div>
...
::: {.notes}
0:28-0:35. Accept variations, but insist that dependencies and inputs precede transformations and outputs.
:::

## One entry point

Name the first script in the README.
...
::: {.notes}
0:35-0:43. Open the provided notebook. Show that students may keep an R script instead. The notebook is optional and ungraded.
:::
...
Substantive correctness requires separate data and interpretation checks.
...
::: {.notes}
0:43-0:49. Expected: missing values yield `NA` without `na.rm=TRUE`; the full table may mix years; the object may not exist in a clean run.
:::
...
- causal interpretation requires another research design.

Name the missing evidence or design feature directly.

::: {.notes}
0:49-0:58. Ask students to improve one generic limitation by naming the exact threat and likely direction of concern.
:::
...
This record still requires details about:
...
::: {.notes}
0:58-1:08. Debrief one example that distinguishes agent generation from human verification. Take a five-minute break.
:::
...
| Evidence | Supports | Further check needed |
|---|---|---|
...
- Can the project recreate every output from its source files?
...
::: {.notes}
1:08-1:23. Assign one audit to each section of the room. Ask each section to name the single most consequential failure in its category and cite the fixture evidence.
:::
...
Repair a broken key before addressing formatting.
...
::: {.notes}
1:23-1:40. Switch roles after five minutes. Authors may only observe. Debrief the first blocker each partner encountered and require a file or output citation.
:::
...
The artifact is optional and ungraded.
...
The goal is work another person can understand, inspect, and improve.

::: {.notes}
1:40-2:00. Run the confidence map, allow ten minutes for one high-value repair, and close with the clean-run receipt. Remind students the artifact is theirs and optional. The fall semester will deepen the statistical methods; this camp establishes the workflow.
:::
